...
async def init_db():
    async with aiosqlite.connect(DB_PATH) as db:
        ...
        """)
        # ✅ Safe migration: add new columns without resetting data
        migrations = [
        ...
        for col_sql in migrations:
            try:
                ...
                pass
        # ✅ Fix bag_kg_max for existing users (set to unlimited if too low)
        await db.execute(
            "UPDATE users SET bag_kg_max=999999.0 WHERE bag_kg_max < 999999.0"
        )
        await db.commit()
    logger.info("✅ DB initialized (v5 FIXED)")
    ...
    async with aiosqlite.connect(DB_PATH) as db:
        await db.execute(
            ...
            (user_id, username or "", first_name or "Miner",
             display_name or first_name or "Miner", STARTING_BALANCE)
        )
        await db.commit()
...
async def get_leaderboard(limit: int = 10) -> list:
    """✅ Leaderboard berdasarkan total_earned (total koin yang pernah didapat)"""
    async with aiosqlite.connect(DB_PATH) as db:
        db.row_factory = aiosqlite.Row
        async with db.execute(
            "SELECT user_id, username, first_name, display_name, total_earned, total_mined, "
            "level, rebirth_count, mine_count "
            "FROM users ORDER BY total_earned DESC LIMIT ?", (limit,)
        ) as cur:
            return [dict(r) for r in await cur.fetchall()]
...
async def get_user_rank(user_id: int) -> int:
    async with aiosqlite.connect(DB_PATH) as db:
        async with db.execute(
            ...
            "(SELECT total_earned FROM users WHERE user_id=?)", (user_id,)
        ) as cur:
            row = await cur.fetchone()
...
async def get_ore_stats(user_id: int) -> list:
    async with aiosqlite.connect(DB_PATH) as db:
        ...
        async with db.execute(
            ...
            "FROM mining_log WHERE user_id=? GROUP BY ore_type ORDER BY cnt DESC",
            (user_id,)
        ) as cur:
            return [dict(r) for r in await cur.fetchall()]
...
async def get_all_users() -> list:
    async with aiosqlite.connect(DB_PATH) as db:
        db.row_factory = aiosqlite.Row
        async with db.execute(
            "SELECT user_id, username, first_name, display_name, balance, level, "
            "total_mined, total_earned, mine_count FROM users"
        ) as cur:
            return [dict(r) for r in await cur.fetchall()]
